...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { rows } = await pool.query('SELECT * FROM adverts WHERE id = $1', [id]);
        if (rows.length === 0) {
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const { rows } = await pool.query(
            ...
            [enterprise_id, title, description, location, start_date, end_date, salary, id]
            );
        if (rows.length === 0) {
            return res.status(404).send('Advert not found');
        }
...
router.delete('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { rows } = await pool.query('DELETE FROM adverts WHERE id = $1 RETURNING *', [id]);
        if (rows.length === 0) {
            return res.status(404).send('Advert not found');
        }
        res.json(rows[0]);
